...
// CCameraControlMode 消息处理程序
GUID EZDEV_H = {0x00873fdf, 0x61a8, 0x11d1, {0xaa, 0x5e, 0x0, 0xc0, 0x4f, 0xb1, 0x72, 0x8b}};	// h version
GUID EZDEV_L = {0xA5DCBF10, 0x6530, 0x11D2, {0x90, 0x1F, 0x00, 0xC0, 0x4F, 0xB9, 0x51, 0xED}};	// libusb version
GUID EZDEV_U = {0x65E8773D, 0x8F56, 0x11D0, {0xA3, 0xB9, 0x00, 0xA0, 0xC9, 0x22, 0x31, 0x96}};	// uvc version
...
*/
	...
	//注册串口监控（OnDeviceChange）
	DEV_BROADCAST_DEVICEINTERFACE NotificationFilter;
	ZeroMemory(&NotificationFilter, sizeof(NotificationFilter));
	NotificationFilter.dbcc_size	   = sizeof(NotificationFilter); 
	NotificationFilter.dbcc_devicetype = DBT_DEVTYP_DEVICEINTERFACE;
	NotificationFilter.dbcc_classguid  = EZDEV_H;
	m_hHDevNotify = RegisterDeviceNotification(m_hWnd, &NotificationFilter, DEVICE_NOTIFY_WINDOW_HANDLE);
	NotificationFilter.dbcc_classguid  = EZDEV_L;
	m_hLDevNotify = RegisterDeviceNotification(m_hWnd, &NotificationFilter, DEVICE_NOTIFY_WINDOW_HANDLE);
	NotificationFilter.dbcc_classguid  = EZDEV_U;
	m_hUDevNotify = RegisterDeviceNotification(m_hWnd, &NotificationFilter, DEVICE_NOTIFY_WINDOW_HANDLE);
...
// 获取USB中断消息
LRESULT CCameraControlMode::OnDeviceChange(WPARAM wParam, LPARAM lParam)
{
	int m_nPCPID = 0;
	int m_nPCVID = 0; 
	...
	if (DBT_DEVTYP_DEVICEINTERFACE == pHdr->dbch_devicetype) 
	{
		...
		m_strID = pDevInf->dbcc_name;
		m_strID.MakeUpper();

		m_nPCPID = FindUSBKomPID(m_strID);
		m_nPCVID = FindUSBKomVID(m_strID);

		if (m_nPCVID == 21587 || m_nPCVID == 21331 || m_nPCVID == 1351 || m_nPCVID == 60321)
		{
			if (m_bWaitting)
			{
				StopWaitForFrame();
			...
			TUCAM_Draw_Uninit(m_opCam.hIdxTUCam);
			TUCAM_Api_Uninit();
			...
			if(m_bLiving && NULL != m_opCam.hIdxTUCam)
			{
				m_bLiving = FALSE;
				StartWaitForFrame();  
			}

			((CSampleProDlg *)(this->GetParent()))->m_PicViewMode.Invalidate();
			UpdateData(FALSE);
...
int CCameraControlMode::FindUSBKomPID(CString str) 
{
	int nPIDPos = str.Find("PID_");

	str.Delete(0, nPIDPos+4);
	str.Delete(4, str.GetLength());
	int nPID = strtol(str, NULL, 16);
	return nPID;
}
int CCameraControlMode::FindUSBKomVID(CString str)
{
	int nPIDPos = str.Find("PID_");

	str.Delete(nPIDPos-1, str.GetLength()-nPIDPos);

	int n_Pos = str.Find("_");
	str.Delete(0, n_Pos+1);
	int nVID = strtol(str, NULL, 16);
