...
While the information is accurate for slower acquisitions the timestamps are still
obtained from the operating systems. For that reason the timestamps may not always
represent the time of actual, in-camera acqusition. This is especially true for fast
CMOS cameras that contain frame buffer.
For accurate hardware timestamps please see @ref Embedded_frame_metadata feature.
*/
...
typedef enum PL_EXPOSURE_MODES
{
    ...
    */
    TIMED_MODE,
    /**
    Causes the external trigger to signal the start of exposure.
    The exposure duration is still controlled by the internal timer.
    */
    STROBED_MODE,
    /**
    Causes the trigger to control the duration of the exposure,
    as well as when it occurs.
    */
    BULB_MODE,
    /**
    Causes the trigger to control the beginning of a sequence.
    Each exposure in the sequence then uses the internal timer to control the
    exposure duration. The software will look for only one trigger and then
...
{
    /**
    Camera will sample and deliver the frames as fast as possible with considering
    ...
    */
    ...
    in next interval (unlike the FPS mode where the sensor would be sampled immediately
    once the buffer/interface is unclogged). This results in even frame intervals but
    there might be gaps	in between the frames deliveries and thus reduced frame rate.
    */
...
The resolution defines in what units is the exposure passed to #pl_exp_setup_seq and
#pl_exp_setup_cont calls and for the Variable Timed Mode (#PARAM_EXP_TIME).
Treated as @c #int32 type.
*/
...
It is recommended to use camera hardware signals for accurate frame synchronization.
...
Similarly the <tt>p1, p2</tt> coordinates corresponds to <tt>y1, y2</tt> coordinates.
...
binaries are usually tighly coupled and cannot be interchanged.
...
In most cases this parameters reports the height of the sensor imaging area or
the number of rows in CMOS type sensor.
...
In most cases this parameters reports the width of the sensor imaging area or
the number of columns in CMOS type sensor.
...
If the mode is set to #PL_SCAN_MODE_PROGRAMMABLE_LINE_DELAY the the
...
object detection modes. Please note that the metadata feature (#PARAM_METADATA_ENABLED)
should be enabled before enabling Centroids feature.
...
- if the camera finds more particles than what is the limit the remaining particles
will be ignored.
...
available, it supports only the #PL_CENTROIDS_MODE_LOCATE feature.
...
For 'particle tracking' mode the value is a fixed-point real number in format Q8.4.
